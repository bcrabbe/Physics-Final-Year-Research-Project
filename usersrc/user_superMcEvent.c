...
#if MC_ENABLE_BASIC_QUALITY_CUTS
...
#if MC_ENABLE_ENERGY_SHARING_CUTS
...
/*** He we find track energy with CPD Corrections ****************************************************/
   ...
   // fprintf(realFP,"%f %f %f %f %f %d %f\n",pi0ReconstructedMass, pi0Pi1Mass, missingMomMag, chargedPartEPRatio,missingMass2,muon,pi0TransMomMag);
    ...
    /*** truth check ***************/
/*    float missing4MomTruth[4]= {evt->decay.p[0],evt->decay.p[1],evt->decay.p[2],evt->decay.p[3]};
	...
	case 4://pi0
        for(int s=0;s<4;++s)
        {
          	missing4MomTruth[s]-=evt->part[i].p[s];
        }
    ...
    case 8://piPlus
       // printf("P pi+ diff:(");
        for(int s=0;s<4;++s)
        {
            // printf("%f, ",chargedPart4Mom[s]-evt->part[i].p[s]);
            missing4MomTruth[s]-=evt->part[i].p[s];
  ...
  //    printf("\n");
	...
	case 512://kaon
	   ...
	   // printf("truth Kaon 4Mom:( %f,%f,%f,%f)\n",evt->decay.p[0],evt->decay.p[1],evt->decay.p[2],evt->decay.p[3]);
	   // printf("beam kaon 4mom: ( %f,%f,%f,%f)\n\n", beam4Mom[0], beam4Mom[1], beam4Mom[2], beam4Mom[3]);
	 // ++kCount;
	  break;
	case (-8)://pi-
	  //++pimiCount;
  ...
  switch (type)
   ...
   // fprintf(ke3FP,"%f\n",missingMassSquared);
    ...
    case K2P_EVENT:
        ++MCk2piCount;
    // fprintf(k2piFP,"%f\n",missingMass2Truth);
      //fprintf(k2piFP,"%f %f %f %f %f %d %f\n",pi0ReconstructedMass, kaonMass, missingMomMag, chargedPartEPRatio,missingMass2,muon,pi0TransMomMag);
      //fprintf(k2piFP,"%f %f %f\n",decayVertex[0],decayVertex[1],decayVertex[2]);
      break;
  case KM2_EVENT:

      //fprintf(km2FP,"%lf\t %lf\n",trackRadiusDCH,zCharged);
      //fprintf(km2FP,"%f %f %f %f %f %d %f\n",pi0ReconstructedMass, kaonMass, missingMomMag, chargedPartEPRatio,missingMass2,muon,pi0TransMomMag);
      //fprintf(km2FP,"%f %f %f\n",decayVertex[0],decayVertex[1],decayVertex[2]);
      break;
case K3P0_EVENT:
        ++MCk3piCount;
     // fprintf(k3pi0FP,"%lf\t %lf\n",trackRadiusDCH,zCharged);
      //fprintf(k3pi0FP,"%f %f %f %f %f %d %f\n",pi0ReconstructedMass, kaonMass, missingMomMag, chargedPartEPRatio,missingMass2,muon,pi0TransMomMag);
      //fprintf(k3pi0FP,"%f %f %f\n",decayVertex[0],decayVertex[1],decayVertex[2]);
      break;
  case K3P_EVENT:
	 //fprintf(k3piFP,"%lf\t %lf\n",trackRadiusDCH,zCharged);
	 //fprintf(k3piFP,"%f %f %f %f %f %d %f\n",pi0ReconstructedMass, kaonMass, missingMomMag, chargedPartEPRatio,missingMass2,muon,pi0TransMomMag);
      //fprintf(k3piFP,"%f %f %f\n",decayVertex[0],decayVertex[1],decayVertex[2]);
      break;
    case UNKNOWN_EVENT:
